...
RUN curl -sL https://deb.nodesource.com/setup | sudo bash - && npm install -g grunt-cli; \
	...
	rm /etc/nginx/sites-enabled/default && mkdir /etc/service/nginx; \
...
COPY 00_regen_sharelatex_secrets.sh 00_make_sharelatex_data_dirs.sh \
			00_set_docker_host_ipaddress.sh /etc/my_init.d/
